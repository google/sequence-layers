...
def mask_invalid(
    self: Sequence[ValuesT, MaskT],
    mask_value: complex | None = None,
) -> Sequence[ValuesT, MaskT]:
  """Returns a sequence with invalid timesteps replaced."""
  expanded_mask = self.expanded_mask()
  if mask_value is None:
    masked_values = mx.zeros_like(self.values)
    result_type: type[Sequence[mx.array, mx.array]] = MaskedSequence
  else:
    masked_values = mx.full(
        self.values.shape,
        mask_value,  # pyrefly: ignore[bad-argument-type]
        self.values.dtype,
    )
