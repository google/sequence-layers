...
class ExportDenseTest(parameterized.TestCase):
  ...
  def test_export_dense_step(self):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
      ...
      # Run native.
      state = layer.get_initial_state(batch_size, input_spec, training=False)
      y_native, _ = layer.step(x, state, training=False)
  ...
  def test_export_dense_no_bias(self):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
      ...
      state = layer.get_initial_state(1, input_spec, training=False)
      y_native, _ = layer.step(x, state, training=False)
...
class ExportAttentionTest(parameterized.TestCase):
  ...
  def test_export_attention_multi_step(self):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
      ...
      native_outputs = []
      for x in inputs:
        y, native_state = layer.step(x, native_state, training=False)
        mx.eval(y.values)
...
class ExportSerialTest(parameterized.TestCase):
  ...
  def test_export_serial(self):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
      ...
      # Native.
      state = model.get_initial_state(batch_size, input_spec, training=False)
      y_native, _ = model.step(x, state, training=False)
