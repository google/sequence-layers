...
def _materialize_deferred(model, batch_size, input_spec, *, constants=None):
  ...
  state = model.get_initial_state(
      batch_size, input_spec, training=False, constants=constants
  )
  model.step(x, state, training=False, constants=constants)
...
def export_step(
  ...
  def step_fn(x_values, x_mask, *state_flat):
    ...
    x = Sequence(x_values, x_mask)
    y, new_state = model.step(x, state, training=False, constants=constants)
