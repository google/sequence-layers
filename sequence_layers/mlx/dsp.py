...
# pylint: disable=protected-access,attribute-defined-outside-init,missing-class-docstring,missing-function-docstring,unsubscriptable-object,no-else-return,unnecessary-lambda
...
class Delay(
  ...
  class Config(spec.Delay.Config):
  ...
  def step(  # pyrefly: ignore[missing-override-decorator]
      self, x: Sequence, state: Sequence, *, training: bool, constants=None
  ):
    if not self.length:
      return x, state
...
class Lookahead(
  ...
  @dataclasses.dataclass(frozen=True)
  class Config(spec.Lookahead.Config):
  ...
  @types.check_step
  def step(  # pyrefly: ignore[missing-override-decorator]
      self, x: Sequence, state: mx.array, *, training: bool, constants=None
  ):
    if not self.length:
      return x, state
    ...
    mask = mx.logical_and(
        x.mask, countdown == 0  # pyrefly: ignore[bad-argument-type]
    )
...
class Window(
  ...
  @dataclasses.dataclass(frozen=True)
  class Config(spec.Window.Config):

    @override
    def make(self) -> 'Window':
      return Window.from_config(self)
...
class Frame(types.PreservesType, types.SequenceLayer, spec.Frame):
  ...
  @dataclasses.dataclass(frozen=True)
  class Config(spec.Frame.Config):

    @override
    def make(self) -> 'Frame':
      return Frame.from_config(self)
  ...
  def step(  # pyrefly: ignore[missing-override-decorator]
    ...
    bw = self._buffer_width
    if bw:
      assert isinstance(state, Sequence)
      state = state.concatenate(x)
    else:
      state = x
...
class OverlapAdd(types.PreservesType, types.SequenceLayer, spec.OverlapAdd):
  ...
  @dataclasses.dataclass(frozen=True)
  class Config(spec.OverlapAdd.Config):

    @override
    def make(self) -> 'OverlapAdd':
      return OverlapAdd.from_config(self)
  ...
  def step(  # pyrefly: ignore[missing-override-decorator]
    ...
    bw = self._buffer_width
    if bw:
      assert isinstance(state, mx.array)
      time = x.shape[1]
...
class FFT(types.Stateless, spec.FFT):
  """Applies FFT to a channel dimension."""

  @dataclasses.dataclass(frozen=True)
  class Config(spec.FFT.Config):

    @override
    def make(self) -> 'FFT':
      return FFT.from_config(self)
...
class IFFT(types.Stateless, spec.IFFT):
  """Applies IFFT to a channel dimension."""

  @dataclasses.dataclass(frozen=True)
  class Config(spec.IFFT.Config):

    @override
    def make(self) -> 'IFFT':
      return IFFT.from_config(self)
...
class RFFT(types.Stateless, spec.RFFT):
  """Applies RFFT to a channel dimension."""

  @dataclasses.dataclass(frozen=True)
  class Config(spec.RFFT.Config):

    @override
    def make(self) -> 'RFFT':
      return RFFT.from_config(self)
...
class IRFFT(types.Stateless, spec.IRFFT):
  """Applies IRFFT to a channel dimension."""

  @dataclasses.dataclass(frozen=True)
  class Config(spec.IRFFT.Config):

    @override
    def make(self) -> 'IRFFT':
      return IRFFT.from_config(self)
...
class InverseSTFT(types.SequenceLayer, spec.InverseSTFT):
  ...
  @dataclasses.dataclass(frozen=True)
  class Config(spec.InverseSTFT.Config):
...
class LinearToMelSpectrogram(
  ...
  @dataclasses.dataclass(frozen=True)
  class Config(spec.LinearToMelSpectrogram.Config):

    @override
    def make(self) -> 'LinearToMelSpectrogram':
      return LinearToMelSpectrogram.from_config(self)
