...
class Conv2DTest(test_utils.SequenceLayerTest):
  ...
  def test_conv2d_dtypes_with_kernel_constraint(
...
class Conv2DTransposeTest(test_utils.SequenceLayerTest):
  ...
  @parameterized.product(
      ...
      compute_dtype=[None, jnp.float32, jnp.bfloat16],
      kernel_constraint=[
          normalization.L2WeightNormalization.Config(),
      ],
  )
  def test_conv2d_transpose_dtypes_with_kernel_constraint(
      self, input_dtype, param_dtype, compute_dtype, kernel_constraint
  ):
    key = jax.random.PRNGKey(1234)
    kernel_size, strides, dilation_rate = 3, 2, 1
  ...
  def test_conv2d_transpose_groups_invalid(self):
    key = jax.random.PRNGKey(1234)
    l = convolution.Conv2DTranspose.Config(
        filters=2,
        kernel_size=(3, 2),
    ...
    batch_size, time, space, channels = 2, 20, 10, 3
    x = test_utils.random_sequence(
        batch_size, time, space, channels, dtype=jnp.float32
    )
