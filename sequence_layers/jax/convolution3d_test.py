...
class Conv3DTest(test_utils.SequenceLayerTest):
  ...
  def test_conv3d(
    ...
    self.assertEqual(l.name, 'conv3d')
    self.assertEqual(
        l.supports_step,
        time_padding
        in (
    ...
    for time in range(20 * l.block_size - 1, 20 * l.block_size + 2):
      with self.subTest(f'time{time}'):
        x = test_utils.random_sequence(
            batch_size, time, spatial1, spatial2, channels
        )
        spatial1_output = utils.convolution_padding_output_size(
            spatial1, spatial_padding, kernel_size, strides, dilation_rate
        )
        spatial2_output = utils.convolution_padding_output_size(
            spatial2, spatial_padding, kernel_size, strides, dilation_rate
        )
        self.assertEqual(
            l.get_output_shape_for_sequence(x),
            (spatial1_output, spatial2_output, filters),
        )
        ...
        # a 3D convolution with the same weights. We manually pad the
        ...
        values_golden = jax.lax.conv_general_dilated(
            x.values,
            kernel,
            window_strides=[strides, strides, strides],
            rhs_dilation=[dilation_rate, dilation_rate, dilation_rate],
            padding=[
        ...
        values_golden = (
            values_golden
            * mask_golden[:, :, np.newaxis, np.newaxis, np.newaxis]
        )
