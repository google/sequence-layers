...
"""Local dot-product self attention layer."""
...
class LocalDotProductSelfAttention(types.Emitting):
  ...
  def setup(self) -> None:
    ...
    if self.config.per_dim_key_scale:
      assert self.config.per_dim_scale
  ...
  def step_with_emits(
    ...
    emits = common.SelfAttentionEmits(
  ...
  def layer_with_emits(
    ...
    context_vectors, probabilities = common.local_dot_product_attention(
    ...
    emits = common.SelfAttentionEmits(types.Sequence(probabilities, x.mask))
