...
def assert_param_dtypes_inits_shapes(
  ...
  if constants is not None:
    if hasattr(config, 'source_names'):
      kv_in_channels = constants[layer._source_names[0]].shape[2]
    else:
...
class DotProductSelfAttentionV2Test(test_utils.SequenceLayerTest):
  ...
  def test_dot_product_self_attention_v2(
      self,
      num_heads,
      units_per_head,
      max_past_horizon,
    ...
    key = jax.random.PRNGKey(1234)
    batch_size = 2
    l = attention.DotProductSelfAttentionV2.Config(
        num_heads=num_heads,
        units_per_head=units_per_head,
        max_past_horizon=max_past_horizon,
        max_future_horizon=max_future_horizon,
        precision=jax.lax.Precision.HIGHEST,
        per_dim_scale=True,
        flash_attention_query_block_size=17,
        flash_attention_key_block_size=31,
        name='dot_product_self_attention_v2',
    ).make()
    ...
    self.assertEqual(l.block_size, 1)
    self.assertEqual(l.output_ratio, 1)
    self.assertEqual(l.name, 'dot_product_self_attention_v2')
    self.assertEqual(
        l.get_output_shape_for_sequence(x), (num_heads, units_per_head)
    )
    self.assertEqual(
  ...
  def test_dtypes(self, param_dtype, input_dtype, compute_dtype, config):
    ...
    l = attention.DotProductSelfAttentionV2.Config(**(defaults | config)).make()
  ...
  def test_use_kv_cache_ringbuffer(
    ...
    l = attention.DotProductSelfAttentionV2.Config(
        ...
        flash_attention_key_block_size=32,
        name='dot_product_self_attention_v2',
    ).make()
...
class BlockwiseDotProductSelfAttentionTest(test_utils.SequenceLayerTest):
  ...
  def test_blockwise_dot_product_self_attention(
    ...
    batch_size = 2
    block_size = 4
    l = attention.BlockwiseDotProductSelfAttention.Config(
        block_size=block_size,
        num_heads=num_heads,
        units_per_head=units_per_head,
...
class MultiSourceDotProductAttentionTest(test_utils.SequenceLayerTest):
  ...
  def test_multi_source_dot_product_attention(self, num_heads, units_per_head):
    ...
    l = attention.MultiSourceDotProductAttention.Config(
