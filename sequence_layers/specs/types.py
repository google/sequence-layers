...
import abc
import dataclasses  # pylint: disable=unused-import
import enum
import fractions
from typing import (
    Any,
...
State = Any
Constants = MutableMapping[str, jt.PyTree[Array]]
...
class Sequence[ValuesT, MaskT](metaclass=abc.ABCMeta):
  ...
  @abc.abstractmethod
  def shape(self) -> Shape:
    """The shape of the sequence as (batch, time, ...channels)."""

  @property
  @abc.abstractmethod
  def ndim(self) -> int:
    """The number of dimensions of the sequence values."""
  ...
  def dtype(self) -> DType:
    """The dtype of the sequence values."""

  @classmethod
  @abc.abstractmethod
  def from_values(cls, values: ValuesT) -> Self:
    """Creates a Sequence from values with a default mask."""
  ...
  def from_lengths(
    ...
    """Creates a Sequence from values and lengths."""
  ...
  def expanded_mask(self) -> Any:
    """Returns the mask expanded to the shape of the values."""
  ...
  def apply_values[NewValuesT: Array, **P](
    ...
    """Applies a function to the sequence values."""
  ...
  def apply_values_masked[NewValuesT: Array, **P](
    ...
    """Applies a function to the sequence values, respecting the mask."""

  @abc.abstractmethod
  def apply[NewValuesT: Array, NewMaskT: Array, **P](
      self,
      apply_fn: Callable[
          Concatenate[ValuesT, MaskT, P], tuple[NewValuesT, NewMaskT]
      ],
      *args: P.args,
      **kwargs: P.kwargs,
  ) -> 'Sequence[NewValuesT, NewMaskT]':
    """Applies a function to both values and mask."""
  ...
  def apply_masked[NewValuesT: Array, NewMaskT: Array, **P](
    ...
    """Applies a function to values and mask, respecting the mask."""

  @abc.abstractmethod
  def astype(self, dtype: DType | None) -> Self:
    """Returns a copy of the sequence with a new dtype."""

  @abc.abstractmethod
  def lengths(self) -> Any:
    """Returns the lengths of the sequences in the batch."""
  ...
  def pad_time(
    ...
    """Pads the sequence along the time dimension."""
  ...
  def mask_invalid(
    ...
    """Returns a MaskedSequence with invalid timesteps zeroed."""

  @abc.abstractmethod
  def unmask(self) -> 'Sequence[ValuesT, MaskT]':
    """Returns a Sequence with no masking applied."""
...
class Steppable(Generic[InputT, OutputT, ChannelSpecT], metaclass=abc.ABCMeta):
  ...
  def block_size(self) -> int:
    """The block size this layer processes at once."""

  @property
  @abc.abstractmethod
  def output_ratio(self) -> fractions.Fraction:
    """The ratio of output timesteps to input timesteps."""

  @property
  @abc.abstractmethod
  def supports_step(self) -> bool:
    """Returns True if the layer supports stepwise processing."""

  @property
  @abc.abstractmethod
  def input_latency(self) -> int:
    """The number of future timesteps required for the current output."""

  @property
  @abc.abstractmethod
  def output_latency(self) -> int:
    """The number of timesteps the output is delayed."""

  @abc.abstractmethod
  def get_accumulated_input_latency(self, input_latency: int) -> int:
    """Calculates the total input latency including previous layers."""

  @abc.abstractmethod
  def get_accumulated_output_latency(self, output_latency: int) -> int:
    """Calculates the total output latency including previous layers."""
  ...
  def layer(
    ...
    Args:
      ...
      constants: A dictionary of constant name to array or sl.Sequence. Values
        or sequences that are "constant" with respect to the SequenceLayer, but
        may affect its processing. For example, for an attention layer this may
        contain the source sequence to attend to.
  ...
  def layer_with_emits(
    ...
    Args:
      ...
      constants: A dictionary of constant name to array or sl.Sequence. Values
        or sequences that are "constant" with respect to the SequenceLayer, but
        may affect its processing. For example, for an attention layer this may
        contain the key/value sequence to attend to.
  ...
  def receptive_field(self) -> Any:
    ...
    For cases where the receptive field varies across steps, this returns the
    union (min/max) of the per-step receptive fields. For example, with 2x
    downsampling and 2x upsampling the receptive field is `(0, 1)` at even
    steps and `(-1, 0)` at odd steps; this property would return `(-1, 1)`.

    Returns:
      A `(start, end)` tuple, or `None`. Infinite receptive field is
      represented with `+/-inf` (e.g. RNNs have `(-inf, 0)`). `None`
      indicates no receptive field (e.g. `Conv1DTranspose` with
      `kernel_size=1, stride=2` produces `None` for every other step).
    """
...
class PreservesType:
  ...
  def get_output_dtype(
    ...
    """Returns the output dtype, which is the same as the input dtype."""
...
class Stateless(SequenceLayer[InputT, OutputT, ChannelSpecT]):
...
class StatelessPointwise(
    PreservesShape,
    Stateless[InputT, OutputT, ChannelSpecT],
...
class StatelessPointwiseFunctor(
    StatelessPointwise[InputT, OutputT, ChannelSpecT]
):
...
class Emitting(SequenceLayer[InputT, OutputT, ChannelSpecT]):
...
class StatelessEmitting(Emitting[InputT, OutputT, ChannelSpecT]):
...
_SequenceLayerConfigType = SequenceLayerConfig
