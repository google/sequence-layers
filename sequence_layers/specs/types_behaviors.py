...
SequenceLayerTest = test_utils_spec.SequenceLayerTest


class ModuleSpecTest(test_utils_spec.ModuleSpecTest):
  """Abstract tests for ModuleSpec behaviors."""
...
class ModuleInterfaceTest(SequenceLayerTest):
  """Abstract tests for ModuleInterface behaviors."""
...
class SequenceTest(SequenceLayerTest):
  ...
  def test_mask_invalid(
    ...
    if expected_mask_value is None:
      output = self.sl.types.Sequence(values, mask).mask_invalid()
      fill_value = 0.0
    else:
      output = self.sl.types.Sequence(values, mask).mask_invalid(mask_value)
  ...
  def test_pad_time(self) -> None:
    ...
    x = self.sl.types.Sequence(values, mask).mask_invalid()
    ...
    x_left1 = self.sl.types.Sequence(
        self.xp.array([
            [0.0, 1.0, 2.0, 3.0, 4.0],
  ...
  def _create_test_sequence(
    ...
    return self.sl.types.Sequence(values, mask)

  def test_slice(self) -> None:
    x = self._create_test_sequence((3, 5, 9))

    self.assertSequencesEqual(
        x[:, 1:], self.sl.types.Sequence(x.values[:, 1:], x.mask[:, 1:])
    )
    self.assertSequencesEqual(
        x[:, ::2], self.sl.types.Sequence(x.values[:, ::2], x.mask[:, ::2])
    )
    self.assertSequencesEqual(
        x[::2, ::3],
        self.sl.types.Sequence(x.values[::2, ::3], x.mask[::2, ::3]),
    )
  ...
  def test_slice_can_slice_channel_dimensions(self) -> None:
    ...
    self.assertSequencesEqual(
        x[:, ::2, :3],
        self.sl.types.Sequence(x.values[:, ::2, :3], x.mask[:, ::2]),
    )
  ...
  def test_apply_values(self) -> None:
    ...
    x = self.sl.types.Sequence(values, mask)
  ...
  def test_from_values(self) -> None:
    ...
    values = self.xp.array(values_np)
    x = self.sl.types.Sequence.from_values(values)
  ...
  def test_mask_invalid_idempotent(self) -> None:
    ...
    self.assertIsInstance(masked, self.sl.types.MaskedSequence)

    masked_again = masked.mask_invalid()
    self.assertIs(masked_again, masked)
    self.assertIsInstance(masked_again, self.sl.types.MaskedSequence)

    masked2 = x.mask_invalid()
    self.assertIsNot(masked2, masked)
    self.assertIsInstance(masked2, self.sl.types.MaskedSequence)
  ...
  def test_from_lengths(self) -> None:
    ...
    mask = self.xp.array(mask_np)
    lengths = self.xp.array(lengths_np)

    x_expected = self.sl.types.Sequence(values, mask)
    x = self.sl.types.Sequence.from_lengths(x_expected.values, lengths)
    ...
    x = self.sl.types.Sequence.from_lengths(
        x_expected.values, self.xp.array([-1, 5, 10, 17, 18])
    )
    self.assertAllEqual(x.lengths(), self.xp.array([0, 5, 10, 17, 17]))
    self.assertNotIsInstance(x, self.sl.types.MaskedSequence)

    # Return type is MaskedSequence if is_masked=True.
    x = self.sl.types.Sequence.from_lengths(
        x_expected.values, self.xp.array([-1, 5, 10, 17, 18]), is_masked=True
    )
    self.assertAllEqual(x.lengths(), self.xp.array([0, 5, 10, 17, 17]))
    self.assertIsInstance(x, self.sl.types.MaskedSequence)
...
class SequenceLayerConfigTest(SequenceLayerTest):
  """Abstract tests for SequenceLayerConfig behaviors."""

  def test_copy(self) -> None:
    backend_sl = self.sl

    @dataclasses.dataclass(frozen=True)
    class Config(backend_sl.types.SequenceLayerConfig):
      """Mock config."""
  ...
  def test_copy_raises_on_non_dataclass(self) -> None:
    ...
    class NonDataclassConfig(backend_sl.types.SequenceLayerConfig):  # pylint: disable=too-few-public-methods
...
class PreservesTypeTest(SequenceLayerTest):
  """Abstract tests for PreservesType behaviors."""
...
class PreservesShapeTest(SequenceLayerTest):
  """Abstract tests for PreservesShape behaviors."""
...
class StatelessTest(SequenceLayerTest):
  """Abstract tests for Stateless layer behaviors."""
...
class EmittingTest(SequenceLayerTest):
  """Abstract tests for Emitting layer behaviors."""
...
class StatelessEmittingTest(SequenceLayerTest):
  """Abstract tests for StatelessEmitting layer behaviors."""
...
class StatelessPointwiseFunctorTest(SequenceLayerTest):
  """Abstract tests for StatelessPointwiseFunctor layer behaviors."""
  ...
  def test_layer_applies_fn_based_on_mask_required(self) -> None:
    for mask_required in [True, False]:
      with self.subTest(mask_required=mask_required):
        ...
        x = self.create_sequence()
        # Mock the apply methods on the Sequence class itself so we return a
        # valid Sequence that satisfies any @check_layer decorators.
        with unittest.mock.patch.object(
