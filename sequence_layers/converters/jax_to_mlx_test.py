...
# pylint: disable=protected-access,redefined-outer-name,no-member,unused-argument,missing-function-docstring,import-outside-toplevel,no-else-return,broad-exception-caught,unused-import,unnecessary-lambda
...
from sequence_layers.converters import jax_to_mlx as weight_converter
...
def _make_mlx_config(config):
  """Convert a JAX config to an MLX config."""
  ...
  if '.' in qual_name:
    ...
    if len(parts) == 2 and parts[1] == 'Config':
      ...
      mlx_module_name = module_name.replace('.jax', '.mlx')
      # Special case for attention: JAX uses a directory, MLX uses a file.
      if '.attention.' in mlx_module_name:
        mlx_module_name = 'sequence_layers.mlx.attention'
      try:
        mlx_module = importlib.import_module(mlx_module_name)
        outer_cls = getattr(mlx_module, outer_class_name)
        mlx_config_cls = getattr(outer_cls, 'Config')

        # Get valid fields for MLX config
        mlx_fields = {f.name for f in dataclasses.fields(mlx_config_cls)}

        # Only copy fields that exist in MLX config
        fields = {
        ...
        def _convert(obj):
          if (
              ...
              and obj.__class__.__name__ == 'Config'
          ):
            return _make_mlx_config(obj)
        ...
        converted_fields = {k: _convert(v) for k, v in fields.items()}
        return mlx_config_cls(**converted_fields)
      except (ImportError, AttributeError):
        pass

  raise NotImplementedError(f'Cannot convert {qual_name} to MLX')


def _make_mlx_model(config):
  """Create an MLX model from a JAX config."""
  from sequence_layers.mlx import utils as mlx_utils

  try:
    return mlx_utils.make_layer(config)
  except Exception:
    mlx_config = _make_mlx_config(config)
...
def _compare_stateless_float(
  ...
  # MLX.
  mlx_model = _make_mlx_model(config)
  x_mx = Sequence(mx.array(values), mx.array(mask, dtype=mx.bool_))
  mlx_out = np.array(mlx_model.layer(x_mx, training=False).values)
...
def _compare_with_constants(
  ...
  x_mx = Sequence(mx.array(values), mx.array(mask, dtype=mx.bool_))
  mlx_out = np.array(
      mlx_model.layer(x_mx, training=False, constants=mlx_constants).values
  )
...
def _compare_step_mode(
  ...
  mlx_state = mlx_model.get_initial_state(
      batch_size, mlx_spec, training=False, constants=state_constants
  )
  ...
  for i in range(num_steps):
    ...
    y_mx, mlx_state = mlx_model.step(
        x_mx, mlx_state, training=False, constants=step_c
    )
    mx.eval(y_mx.values)
...
class StepModeStreamingAttentionTest(parameterized.TestCase):
  ...
  @unittest.skip('StreamingLocalDotProductAttention not implemented in MLX')
...
def _compare_parametric_float_masked(
  ...
  x_mx = Sequence(mx.array(values), mx.array(mask, dtype=mx.bool_))
  mlx_out = mlx_model.layer(x_mx, training=False)
...
def _compare_integration_float(
  ...
  x_mx = Sequence(mx.array(values), mx.array(mask, dtype=mx.bool_))
  mlx_out = mlx_model.layer(x_mx, training=False, constants=mlx_constants)
  mlx_values = np.array(mlx_out.values)
...
def _compare_integration_step(
  ...
  mlx_spec = ShapeDType(input_shape, mx.float32)
  mlx_state = mlx_model.get_initial_state(
      batch_size, mlx_spec, training=False, constants=mlx_constants
  )
  mlx_outputs = []
  for i in range(num_steps):
    x_mx = Sequence(
    ...
    y_mx, mlx_state = mlx_model.step(
        x_mx, mlx_state, training=False, constants=mlx_constants
    )
    mx.eval(y_mx.values)
    mlx_outputs.append(np.array(y_mx.values))
...
def _compare_integration_int_step(
  ...
  mlx_state = mlx_model.get_initial_state(batch_size, mlx_spec, training=False)
  mlx_outputs = []
  for i in range(num_steps):
    x_mx = Sequence(
    ...
    y_mx, mlx_state = mlx_model.step(x_mx, mlx_state, training=False)
    mx.eval(y_mx.values)
    mlx_outputs.append(np.array(y_mx.values))
